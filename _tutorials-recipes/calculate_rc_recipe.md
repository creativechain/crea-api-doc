...
Since HF20 a Resource Credit (RC) system has been implemented to manage the number of transactions (comments, votes, transfers, etc) you can execute on the blockchain at any given time. This recipe will look at how to calculate your current RC and also what the current RC cost is for a given transaction. This recipe is far more 'basics oriented' than most. For a more in-depth description of how RC's work consume [this excellent RC demo](https://github.com/creativechain/rcdemo) created by Creary's Blockchain Team.

## Intro

RCs are non-transferable credits that accrue to each Crea account based on how much Crea Power(SP) it has. An account spends RC when it transacts on the Crea blockchain. RCs regenerate over a 5 day period. If an account doesn’t have sufficient credits, the transaction will not be allowed to occur.
...
There are applications available to check an account's status, like [cread.com](https://cread.com/@username), but here we will look at how calculate the values manually.
...
Additional info can also be found in [this article by crea user @holger80](https://creary.net/utopian-io/@holger80/update-for-beem-adding-rc-costs-calculation-and-witnesssetproperties-broadcasting?sort=new)

## Allocation of RC to blockchain resources

An in depth look at how RC's are assigned to each of the three resources (CPU megacycles/state memory/history size) can be found in Creary's wiki articles for [RC Bandwidth System](https://github.com/creativechain/crea/wiki/RC-Bandwidth-System) and [Parameters](https://github.com/creativechain/crea/wiki/RC-Bandwidth-Parameters)